...
const HomeScreen = ({ navigation, route }: HomeProps) => {
  const [barbers, setBarbers] = useState<BarberContainerProps[]>([]);

  const getBarbersFromDatabase = async () => {
    const barbersFromDatabase = await getBarbers();
    setBarbers(barbersFromDatabase);
  };

  useEffect(() => {
    if (!barbers.length) {
      getBarbersFromDatabase();
    }
  }, []);
  ...
  return (
    <View className="box-border flex-1 p-0 m-0 bg-gray-950">
      ...
      <View className="w-full mx-auto my-0 mt-2 border max-w-90 ">
        ...
        {barbers.map((barber) => (
          <BarberContainer {...barber} key={barber.id} />
        ))}
      </View>
    </View>
  );
